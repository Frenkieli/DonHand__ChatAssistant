...
class LineMessage extends LineBase{
    /**
   * @description line message event
   * @author frenkie
   * @date 2020-08-11
   * @param {lineEvent} event
   * @returns 
  */
  public async onMessage (event: lineEvent) {
    console.log('訊息事件發生');
    console.log(event);
    // use reply API
        ...
        switch (keyWord) {
          ...
          case '.y2b':
            ...
            request(options, function (error: any, response: any) {
              ...
              vm.client.replyMessage(event.replyToken, replyMessage).catch((err: any) => {
...
export default lineMessageItem.onMessage.bind(lineMessageItem);